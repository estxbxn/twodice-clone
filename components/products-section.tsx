  ...
  return (
    <section className="w-full">
      <div className="flex items-end justify-between">
        <div>
          <h2 className="text-lg md:text-xl font-semibold">Latest Products</h2>
          <p className="text-muted-foreground text-xs md:text-sm">Latest and bestsellers of the week.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        {items.map((item, idx) => (
          <ProductCard variant="flat" accent="red" key={idx} {...item} />
        ))}
      </div>
    </section>
  );
